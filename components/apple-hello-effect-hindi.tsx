...
> & {
  ...
  /**
   * Controls when the handwriting animation starts.
   * @defaultValue true
   */
  play?: boolean
  /** Optional scroll progress value that scrubs the handwriting animation. */
...
function HelloPath({
  ...
  const progress = drawProgress ?? fallbackProgress
  const start = delay / 5.4
  const end = (delay + duration) / 5.4
  ...
  return (
    <motion.path
      ...
      animate={play ? animateProps : initialProps}
      transition={{
        duration: duration * durationScale,
        ease: "easeOut",
        delay: delay * durationScale,
        opacity: { duration: 0.2, delay: delay * durationScale },
      }}
      onAnimationComplete={onAnimationComplete}
    />
  )
    ...
    >
      ...
      {helloPaths.map((path, index) => (
        <HelloPath
          ...
          play={play}
          onAnimationComplete={
            index === helloPaths.length - 1 ? onAnimationComplete : undefined
          }
        />
      ))}
    </motion.svg>
