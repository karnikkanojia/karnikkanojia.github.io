...
export function AnimatedGradient({
  ...
  const params = useMemo((): PresetParams => {
    ...
    return {
      ...
      softness: config.softness ?? preset.softness,
    }
  }, [config])
  ...
  useEffect(() => {
    ...
    void main() {
      ...
      animationTimeRef.current = initialTimeSeconds
      lastFrameTimeRef.current = undefined
      ...
      const renderFrame = (time: number) => {
        ...
        if (
          ...
          animationTimeRef.current += (time - lastFrameTimeRef.current) / 1000
        }
      ...
      const resizeObserver = new ResizeObserver(() => {
        if (!shouldAnimateRef.current && !resizeWhilePaused) return
        resize()
        if (!shouldAnimateRef.current && frameIdRef.current === undefined) {
          frameIdRef.current = requestAnimationFrame(renderFrame)
        }
      })
